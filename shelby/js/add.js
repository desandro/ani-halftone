...
function render() {
  frame++;
  // renderGrid( Math.PI / 3, 'red', 30, 'square' );
  // renderGrid( -1.5 , 'green', 24, 'circle' );
  // renderGrid( -1.5 + Math.PI / 4, 'blue', 14, 'line' );
  renderGrid( 1 + TAU / 3, 'red', 20, 'circle' );
  renderGrid( 1 + TAU * 2 / 3, 'green', 24, 'circle' );
  renderGrid( 1, 'blue', 14, 'circle' );
...
function renderGrid( angle, color, spacing, shape ) {
  ...
  switch ( color ) {
    case 'red' :
      ...
      break;
    case 'green' :
      renderCtx.fillStyle = isAdditive ? 'rgb(0,0,255)' : 'rgb(255,0,255)';
      break;
    case 'blue' :
      renderCtx.fillStyle = isAdditive ? 'rgb(0,255,0)' : 'rgb(255,550,0)';
      break;
  }

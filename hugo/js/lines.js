...
function render() {
  frame++;
  renderGrid( 0, 'red', 16 );
  renderGrid( Math.PI * 2/3, 'green', 12 );
  renderGrid( -Math.PI * 2/3, 'blue', 26 );
...
function renderGrid( angle, color, spacing ) {
  ...
  for ( var row = 0; row < rows; row++ ) {
    for ( var col = 0; col < cols; col++ ) {
      ...
      if ( x2 > 0 && x2 < w && y2 > 0 && y2 < h ) {
        ...
        // render dot
        var size = colorSize * radius;
        renderCtx.save();
        renderCtx.translate( x2, y2 );
        renderCtx.rotate( angle );
        renderCtx.fillRect( -size * 0.5, -spacing, size, spacing+1 );
        renderCtx.restore();
        
        // renderCtx.save();
        // ctx
        // renderCtx.fillRect( x2, y2, colorSize * radius, colorSize * radius );
